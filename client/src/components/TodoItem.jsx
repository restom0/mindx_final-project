    ...
    <article className={`todo-item todo-item--${priority} ${todo.completed ? "todo-item--done" : ""}`}>
      ...
      <div className="todo-item__content">
        <div className="todo-item__title-row">
          <h3>{todo.title}</h3>
          <span
            className={`priority-pill priority-pill--${priority}`}>{t(`advanced.priority${priority[0].toUpperCase()}${priority.slice(1)}`)}</span>
        </div>
        <p>{todo.description || t("todo.noDescription")}</p>
        <div className="todo-item__meta">
          <span>{t("todo.version", {version: todo.version})}</span>
          {todo.dueDate ? (
          ...
          {todo.recurrenceType && todo.recurrenceType !== "NONE" ?
            <span>{todo.recurrenceType.toLowerCase()}</span> : null}
